...
class CourseItemsController < ApplicationController
  ...
  def reschedule
    ...
    # one — otherwise dragging a course item's due date in the day view flips it to weekly.
    ...
    render turbo_stream: [
      turbo_stream.replace(
        "dashboard_calendar",
        partial: "dashboard/calendar_frame",
        locals: { events: occurrences, start_date: start_date, view: (daily ? "daily" : nil) }
      ),
      turbo_stream.replace("agenda_list", partial: "agenda/list"),
      turbo_stream.update("event_popover", "")
    ]
